...
const config = {
    canvasWidth: 400,
    canvasHeight: 400,
    defaultBackgroundImage: path.resolve(`${process.cwd()}/Fonts/image.png`),  // الخلفية الافتراضية
    statusColors: {
        online: '#43b581',
        idle: '#faa61a',
        dnd: '#f04747',
    ...
    run: async (client, message) => {
        try {
            ...
            try {
                ...
                return message.channel.send("Error loading background image. Please make sure the URL is valid.");
            }

            // رسم اللوحة اليسرى
            drawLeftPanel(ctx, canvas);

            // رسم صورة الملف الشخصي و الحالة
            ...
            message.channel.send("❌ An error occurred while generating the profile card.");
        }
    }
};
